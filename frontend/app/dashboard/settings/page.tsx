...
export default function SettingsPage() {
  ...
  const saveReminderPreference = async () => {
    ...
    try {
      if (dailyReminderEnabled && notificationSupported && Notification.permission === 'default') {
  ...
  const exportDataAsPdf = async () => {
    ...
    try {
      ...
      y += 4
      doc.setFontSize(10)
      doc.text('Generated by Nova — your data is encrypted on your device before being stored; we can never read it', 14, y)
  ...
  const handleClearData = async () => {
    ...
    try {
      await api.delete(`/api/cycles/${userId}`)
      clearStoredAuthSession()
      clearOnboardingDraft()
      router.replace('/login')
    } catch (err) {
      if (axios.isAxiosError(err) && err.response?.status === 401) {
        ...
        toast.error('Could not clear backend data right now')
      }
    } finally {
      ...
      {showPermissionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-lg border border-border bg-card p-5 space-y-4">
            <h3 className="text-base font-medium text-foreground">Enable daily reminder notifications?</h3>
            <p className="text-sm text-muted-foreground">
              Nova uses browser notifications to send your daily pill reminder at the time you choose. We only schedule reminders if you allow notifications.
            </p>
            <div className="flex gap-2 justify-end">
              <button
                type="button"
                onClick={() => setShowPermissionModal(false)}
                className="px-3 py-2 rounded-lg border border-border text-foreground hover:bg-muted transition-colors"
              >
                Not now
              </button>
              <button
